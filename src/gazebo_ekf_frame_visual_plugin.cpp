#include "gazebo_ekf_frame_visual_plugin.h"

using namespace gazebo;

GZ_REGISTER_VISUAL_PLUGIN(EkfFrameVisualPlugin)

EkfFrameVisualPlugin::EkfFrameVisualPlugin() {}

EkfFrameVisualPlugin::~EkfFrameVisualPlugin() {}

void EkfFrameVisualPlugin::Load(rendering::VisualPtr _visual,
                             sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_visual, "EkfFrameVisualPlugin _visual pointer is NULL");
  GZ_ASSERT(_sdf, "EkfFrameVisualPlugin _sdf pointer is NULL");

  this->visual = _visual;
  this->sdf = _sdf;

  gzdbg << "Loading EkfFrameVisualPlugin plugin" << std::endl;
}

void EkfFrameVisualPlugin::Init()
{
  ...
  if (this->sdf->HasElement("topic_name")) {
      ...
      this->subs = this->node->Subscribe("~/" + lift_force_topic, &EkfFrameVisualPlugin::OnUpdate, this);
...
void EkfFrameVisualPlugin::OnUpdate(ConstForcePtr& force_msg)
{
...
void EkfFrameVisualPlugin::UpdateVector(const ignition::math::Vector3d& center, const ignition::math::Vector3d& force)
{
